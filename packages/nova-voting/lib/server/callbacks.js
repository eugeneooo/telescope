...
function updateUser (item, user, collection, operation) {
  ...
  switch (operation) {
    case "upvote":
      update.$addToSet = {[`${Users.prefix}upvotedPosts`]: vote};
      break;
    case "downvote":
      update.$addToSet = {[`${Users.prefix}downvotedPosts`]: vote};
      break;
    case "cancelUpvote":
      update.$pull = {[`${Users.prefix}upvotedPosts`]: {itemId: item._id}};
      break;
    case "cancelDownvote":
      update.$pull = {[`${Users.prefix}downvotedPosts`]: {itemId: item._id}};
      break;
  }
...
function updateKarma (item, user, collection, operation) {
  ...
  if (item.userId !== user._id) {
    Users.update({_id: item.userId}, {$inc: {[`${Users.prefix}karma`]: karmaAmount}});
  }
